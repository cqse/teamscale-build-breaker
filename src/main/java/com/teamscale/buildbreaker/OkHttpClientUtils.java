...
public class OkHttpClientUtils {
    ...
    private static void configureTrustStore(OkHttpClient.Builder builder, String trustStorePath, String trustStorePassword) {
        ...
        try {
            ...
            if (trustManagerFactory.getTrustManagers().length == 0) {
                throw new TeamscaleFeedbackInternalException("No trust managers found. This is a bug. Please report it to CQSE.");
            }

            applySslContextAndTrustManager(builder, sslContext, trustManagerFactory);
        } catch (NoSuchAlgorithmException e) {
            throw new TeamscaleFeedbackInternalException("Failed to instantiate an SSLContext or TrustManagerFactory: " + e.getMessage() +
                    "\nThis is a bug. Please report it to CQSE.", e);
        } catch (KeyStoreException e) {
            throw new TeamscaleFeedbackInternalException("Failed to initialize the TrustManagerFactory with the keystore: " + e.getMessage() +
                    "\nThis is a bug. Please report it to CQSE.", e);
        } catch (KeyManagementException e) {
            throw new TeamscaleFeedbackInternalException("Failed to initialize the SSLContext with the trust managers: " + e.getMessage() +
                    "\nThis is a bug. Please report it to CQSE.", e);
        }
    }
    ...
    private static void applySslContextAndTrustManager(OkHttpClient.Builder builder, SSLContext sslContext, TrustManagerFactory trustManagerFactory) {
        try {
            ...
            throw new TeamscaleFeedbackInternalException("Trust manager is not of X509 format: " + e.getMessage() +
                    "\nThis is a bug. Please report it to CQSE.", e);
        }
    }
    ...
    private static KeyStore readKeyStore(String keystorePath, String keystorePassword) {
        try (FileInputStream stream = new FileInputStream(keystorePath)) {
            ...
            throw new KeystoreException("Failed to load one of the certificates in the keystore file " + keystorePath + ": " + e.getMessage() +
            ...
            throw new TeamscaleFeedbackInternalException("Failed to instantiate an in-memory keystore: " + e.getMessage() +
                    "\nThis is a bug. Please report it to CQSE.", e);
        }
    }
